...
function decodeActivationToken(token: string) {
  try {
...
export function ActivateAccountPage() {
  ...
  async function activateAccount(nextEmail = email, nextOtp = otp) {
  ...
  useEffect(() => {
    ...
    if (token) {
      const decodedToken = decodeActivationToken(token)
      ...
      <AuthCard eyebrow="Activation relay" title="Confirm your account link.">
        <div className="mb-8">
          ...
          <p className="mt-2 text-sm leading-6 text-ink/65 dark:text-paper/65">
            Activation links can prefill the email and OTP, then submit
